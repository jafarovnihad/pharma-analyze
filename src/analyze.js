...
function analyze({ product, markets, today = new Date() }) {
  ...
  for (let round = 0; round < MAX_ROUNDS; round++) {
    ...
    if (moves.length === 0) break; // settled: no further beneficial move
...
// Apply a move to the working copy exactly as app.js executes it: take qty FEFO
// from the source and append one new batch at the destination per consumed
// source batch (preserving expiry). Mutates `working`.
function applyFefoMove(working, mv) {
  ...
  for (const b of fefoSort(from.batches)) {
